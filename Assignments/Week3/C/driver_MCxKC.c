...
int main(int argc, char *argv[])
{
  ...
  printf( "data = [ \n" );
  printf( "%%  n   MC   KC        reference      |         current implementation \n" );
  printf( "%%               time       GFLOPS   |    time       GFLOPS     diff \n" );
  for ( MC=MR; MC<=512; MC+=MR ){
    for ( KC=4; KC<=512; KC+=4 ){    
      i = 1;

      for ( size=last; size>= last; size-=inc ){
	...
	*/
	...
	printf( " %5d %3d %3d %8.4le %8.4le   ", n, MC, KC, dtime_best, gflops/dtime_best );
  ...
  printf( "title( \'m=n=k=%d, MRxNR = %d x %d \', \'FontSize\', 16 );\n", n, MR, NR );

		   ...
		   double *B, int ldB, double *C, int ldC )
{
  ...
  /* Resize NC so that each thread uses at most MC/max_threads.
  ...
  int loadbalanced_part = ( n / ( NC_per_thread * max_threads ) )
                          * NC_per_thread * max_threads;
  ...
  for ( int j=0; j<loadbalanced_part; j+=NC_per_thread )
